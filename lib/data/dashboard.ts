...
const DEMO_PF = [1284.6, 1258.2, 1219.8, 1191.4, 1178.9, 1155.2, 1139.6, 1118.4, 1094.7, 1068.2, 1019.5, 987.3];
const DEMO_PA = [1098.3, 1142.1, 1104.8, 1165.9, 1190.3, 1128.6, 1177.4, 1202.1, 1118.6, 1214.5, 1240.9, 1227.7];
...
function demoData(): DashboardData {
  const standings = DEMO_NAMES.map(([teamName, ownerName], index) => ({
    ...
    streak: ['W4', 'W2', 'L1', 'W3', 'L2', 'W1', 'L1', 'W1', 'L3', 'W1', 'L5', 'L2'][index],
    rank: index + 1,
  }));

  const scores = [[132.8, 118.4], [104.2, 110.7], [96.4, 95.9], [121.5, 124.1], [87.7, 101.3], [113.2, 92.6]];
  ...
  return {
    ...
    matchups,
    activities: [
      { id: 'a1', type: 'trade', title: 'Two-team blockbuster', detail: 'Gridiron Ghosts received C. Lamb · The Red Zone received B. Hall + a 2027 2nd', time: '18m' },
      { id: 'a2', type: 'waiver', title: 'Waiver claim · $17 FAAB', detail: 'Punt Intended added T. Benson and dropped R. Dowdle', time: '3h' },
      { id: 'a3', type: 'free_agent', title: 'Free agent move', detail: 'Sunday Scaries added SEA D/ST', time: '5h' },
    ],
    trending: [
      { id: 'p1', name: 'TreVeyon Henderson', position: 'RB', team: 'NE', count: 12491, available: true },
      { id: 'p2', name: 'Xavier Worthy', position: 'WR', team: 'KC', count: 9724, available: false, rosteredBy: 'Fourth & Long' },
      { id: 'p3', name: 'Colston Loveland', position: 'TE', team: 'CHI', count: 7391, available: true },
    ],
    narratives: [
...
function score(roster: SleeperRoster, key: 'fpts' | 'fpts_against') {
  return Number(`${roster.settings[key]}.${roster.settings[`${key}_decimal` as keyof typeof roster.settings] ?? 0}`);
}
...
function createMatchups(matchups: SleeperMatchup[], standings: TeamStanding[], live: boolean) {
...
function addWeeklyPerformance(
  ...
  for (const week of weeks) {
    ...
    for (const entry of valid) {
      const record = medianRecords.get(entry.roster_id) ?? { wins: 0, losses: 0 };
      if (entry.points >= median) record.wins += 1;
      else record.losses += 1;
      medianRecords.set(entry.roster_id, record);
    }
    const groups = new Map<number, SleeperMatchup[]>();
    for (const entry of valid) groups.set(entry.matchup_id!, [...(groups.get(entry.matchup_id!) ?? []), entry]);
    for (const pair of groups.values()) {
      if (pair.length !== 2) continue;
      const first = pair[0].points === pair[1].points ? 'T' : pair[0].points > pair[1].points ? 'W' : 'L';
      results.set(pair[0].roster_id, [...(results.get(pair[0].roster_id) ?? []), first]);
      results.set(pair[1].roster_id, [...(results.get(pair[1].roster_id) ?? []), first === 'W' ? 'L' : first === 'L' ? 'W' : 'T']);
    }
  }
  ...
  return standings.map((team) => {
    ...
    let streak = 0;
    for (let index = teamResults.length - 1; index >= 0 && teamResults[index] === last; index -= 1) streak += 1;
    return {
      ...team,
...
function playerName(playerId: string, players: Record<string, SleeperPlayer>) {
  return players[playerId]?.full_name ?? players[playerId]?.last_name ?? playerId;
}
...
function createActivities(
  ...
  return transactions
    ...
    .slice(0, 6)
    .map((transaction) => {
      const added = Object.keys(transaction.adds ?? {}).map((id) => playerName(id, players));
      const dropped = Object.keys(transaction.drops ?? {}).map((id) => playerName(id, players));
      const actors = transaction.roster_ids.map((id) => byRoster.get(id)).filter(Boolean).join(' & ');
      const detail = [added.length ? `added ${added.join(', ')}` : '', dropped.length ? `dropped ${dropped.join(', ')}` : ''].filter(Boolean).join(' · ');
      return {
        id: transaction.transaction_id,
        type: transaction.type === 'trade' ? 'trade' : transaction.type === 'waiver' ? 'waiver' : 'free_agent',
        title: transaction.type === 'trade' ? `Trade · ${actors}` : actors || 'League transaction',
        detail,
        time: new Intl.DateTimeFormat('en', { month: 'short', day: 'numeric' }).format(new Date(transaction.created)),
      };
    });
}
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  try {
    ...
    const byRoster = new Map(standings.map((team) => [team.rosterId, team.teamName]));
    ...
    return {
      ...
      week,
      statusLabel: league.status === 'in_season' ? `Week ${week} · Live league data` : `Week ${week} · ${league.status.replace('_', ' ')}`,
      ...
      narratives: [
        high ? `${high[0]} set this week’s pace with ${high[1].toFixed(1)} points.` : `Week ${week} is ready for kickoff.`,
